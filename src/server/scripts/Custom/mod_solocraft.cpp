    ...
    private:
        ...
        void ApplyBuffs(Player *player, Map *map, float difficulty, int numInGroup)
        {
            ...
            if (difficulty != 0)
            {
                ...
                {
                    // Buff the player
                    //player->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_PCT, difficulty * 100.0, true);//org
                    player->HandleStatFlatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_VALUE, difficulty * 100.0, true);
        ...
        void ClearBuffs(Player *player, Map *map)
        {
            ...
            if (unitDifficultyIterator != _unitDifficulty.end())
            {
                ...
                for (int32 i = STAT_STRENGTH; i < MAX_STATS; ++i)
                {
                    //player->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_PCT, difficulty * 100.0, false);//org
                    player->HandleStatFlatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_VALUE, difficulty * 100.0, false);
                }
            }
        }
    };
